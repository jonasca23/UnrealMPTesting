...
{
	// Clear Find Sessions Complete delegate handle on SessionInterface
	if (SessionInterface)
	{
		SessionInterface->ClearOnFindSessionsCompleteDelegate_Handle(FindSessionsCompleteDelegateHandle);
	}

	// In case our SearchResults doesn't have any session, Broadcast it's failure
	if (LastSessionSearch->SearchResults.Num() <= 0)
	{
		MultiplayerOnFindSessionsComplete.Broadcast(TArray<FOnlineSessionSearchResult>(), false);
	}

	// At least, if everything went right, broadcast telling our search results
